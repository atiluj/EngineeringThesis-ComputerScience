...
Jeśli chcemy wydrukowac tabelę, wynik kwerendy czy zestawinie zawierające kilka tabel w przejrzysty sposób, 
...
nie jest umiejętność przydatna na maturze, więc i zadanie będzie typowo życiowe, a nie maturlane. <br/><br/>
...
Owszem, jest, sprawdźmy więc jak będzie wyglądała nasza tabela, jeśli po prostu wyfukujemy ją wporst.
Możemy to zrobić poprzez wybranie zakładni "Dane zewnętrzne" -> "PDF lub XPS" 
...
Zauważmy jednak, że efekt nie jest zadowalaujący.
<img src={new URL('https://i.imgur.com/7rQyyoI.png', import.meta.url)} alt="xyz" className={`medium_img`} />
...
### 2. Wybieramy tabelę, na podstwie której będziemy towrzyć raport oraz wszystkie jej pola, które mają się w nim znaleźć
(Wyboru dokonujemy dokłądnie tak samo, jak przy towrzeniu kwerent, poprzez podwójne kliknięcie lub użycie ">".)
Nas interesuje wydrukowanie wszystkich danych o`Kompyterach`, wybieramy więc odpowiednia tabelę oraz wszystkie jej pola.
...
Podobnie do sortowania wielopoziomowego w Excelu, najpierw wszytskie dane będą posegregowane według pola, 
które wybraliśmy w poprzednim kroku, a następnie według kolejnych pól wybranych w tym kroku, zgodnie z kolejnością wyboru. 
Domyślenie porządek jest ustawiony rosnąco, ale można zmienić go na malejący poprzez kliknięcie na tę opcję.
...
Mamy także opcję podsumowania, dzięki której możemy rozrzerzyć raport o sumę czy średnia. 
**Na przykład** dodamy informację o łącznej poejmność wszystkich dysków w sprzętach z tablicy `Komputery`.
<img src={new URL('https://i.imgur.com/90GnySq.png', import.meta.url)} alt="xyz" className={`medium_img`} />

### 5. Wybieramy układ raportu.
Dostepne są trzy równe układy. Klikając na każdy z nich możemy sprawdzić na podglądzie po lewej stronie, 
jak będzie wyklądał nasz raport. Jeśli raport okaże się zbyt szeroki, bo uwzględniliśmy w nim wiele danych 
...
### 8. Widok projektu.
Klikamy w opcję "Widok". Widzimy tu kilka różych trybów, w jakich możemy oglądać nasz raport (podobnie jak w przypadku kwerend). 
...
Na przykład wybierzemy błękitne tło do nazwy Sekcji, a wiec głównego kryterium, według którego sortujemy dane.
...
"Projekt raportu" -> "Widok" -> "Pidgląd wydruku".
