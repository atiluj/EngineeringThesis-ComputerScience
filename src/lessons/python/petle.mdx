...
for i in range(4, 10, 2):
    print(i)
```
Powyższy kod wypisze na ekranie liczby parzyste od 4 do 8 (bo 10 nie jest włączne, a krok ustawiliśmy na 2).
...
```
W powyższym przykładzie przechodzimy po elementach listy odwołując się do jej indeks. **range(len(studenci))** generuje indeksy od 0 aż do długości tablicy - 1 (włącznie).<br/>
...
```
W powyższym przykładzie wszystko zadziałało tak jak się tego spodziewaliśmy. Co jednak jeśli użyjemy pętli for na drugi poznany już sposób:
```python
...
```
Chwila, otrzymujemy niezmodyfikowaną tablicę. Ale dlaczego? 
...
### Pętla w pętli
Zagnieżdzanie pętli polega na umieszczeniu jednej pętli wewnątrz drugiej. W Pythonie jest to często przydatne, szczególnie przy pracy z danymi posiadającymi wiele poziomów zagnieżdżenia. <br/><br/> Przykładem może być pętla zewnętrzna poruszająca się po wierszach, a wewnętrzna po kolumnach listy dwuwymiarowej.
```python
